...
/**
 * ラン分割のしきい値は **`core/settings.ts` とは別に持つ**。
 *
 * 一見すると重複なので共有したくなるが、**実測すると悪化する**。
 * 解析側を「誤答 0」の設定（edgeΔE 20 / clusterΔE 24 / minWidth 2）に
 * 変えたとき、較正側も同じ値に揃えると:
 *
 * - 対応付けの成立は 30/39 → 33/39 に**増える**
 * - しかし学習されたパレットが変わり、読み取りは
 *   正解 27・誤答 0 → 正解 27・**誤答 2** に**悪化**する
 *
 * 較正は「取りこぼさない」ことが目的で細かく割りたいのに対し、読み取りは
 * 「余計に割らない」ことが目的。目的が逆なので値も別でよい。
 * パレットと変換は密結合していて、較正の質が上がることは読み取りの質が
 * 上がることを意味しない（`docs/09-色空間と色差指標の検討.md`）。
 */
