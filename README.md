...
番号と色名・意味は、**水平な抵抗器なら下、垂直なら右**に出します（文字が
色帯に重なると読めなくなるため）。

色空間は「惜しかったのか、まるで違うのか」を見るためのものです。1 行が
RGB、もう 1 行が CIE L\*a\*b\* で、それぞれ**立体 1 面 + 平面 2 面**を並べます。

- **基準色**は色名（`白` `金` …）を**その色で**置きます
- **実測色**は写真に焼いた**通し番号**を**その色で**置きます

番号を突き合わせれば、どのバンドがどの基準色へ引き寄せられたかを追えます。

| 空間 | 立体 | 平面 |
| ------ | ------ | ------ |
| RGB | R-G-B の立方体 | `B-R`、`G-B` |
| CIE L\*a\*b\* | a\*-L\*-b\* の直方体 | `b*-a*`、`b*-L*` |

平面の軸は**先に書いたほうが横**です（`B-R` なら横が B、縦が R）。立体は
散らばりを掴むのに向きますが、重なった点の前後関係が読めません。平面図を
並べると「どの成分で近いのか」がはっきりします。

立体を 3 軸のまま描くのは、a\*b\* 平面だけだと黒・灰・白・銀が原点に重なり、
いちばん取り違えやすい組が見えなくなるためです。
